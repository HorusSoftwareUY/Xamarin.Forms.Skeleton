...
- **A view holding a single piece of content now fades that content while busy**, as every `Layout`
  ...
  `ContentView`, `ScrollView`, `RefreshView` and `SwipeView` are the usual ones. Templated
  controls take the same path, where what gets faded is the control template's root rather than
  the `Content` property. This is the one change that can alter how an existing app looks: if
  content that used to stay visible now disappears, set `sk:Skeleton.IsParent="True"` on that
  container to get the old behaviour back.
